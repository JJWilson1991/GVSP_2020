...
ar<-unique(nest_o$data[[1]]$Host_species)
ca<-unique(nest_o$data[[2]]$Host_species)%>%as.factor()
ch<-unique(nest_o$data[[3]]$Host_species)
la<-unique(nest_o$data[[4]]$Host_species)
pe<-unique(nest_o$data[[5]]$Host_species)
ro<-unique(nest_o$data[[6]]$Host_species)

Man<-c("red","orange","yellow","green", "blue", "purple")
names(Man)<-c(ar,ca,ch,la,pe,ro)  
...
`Domestic rabbits`="pink", `Wild rabbits`="pink",
Horse= "yellow",
`Norway rats`= "blue") 
...
bipartite_D3(pp, ColourBy = 1, colouroption = "manual", NamedColourVector = ManualColours, PrimaryLab = "Reservoir Host", SecondaryLab = "Spillover Host", SiteNames = "", SortPrimary = hi_host, SortSecondary = lo_host, filename = "Species_net")
```
```{r}
...
bipartite_D3(sst2, ColourBy = 1, colouroption = "brewer", PrimaryLab = "Reservoir Host Order", SecondaryLab = "Spillover Host Order", SiteNames = "", filename = "Order_net")
